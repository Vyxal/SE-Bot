...
export default {
  ...
  promote: async (args, reply, message) => {
    ...
    if (isNaN(userId)) {
      ...
      return reply + "That user is already an admin (higher than privileged).";
  ...
  admin: async (args, reply, message) => {
    ...
    if (isNaN(userId)) {
      ...
      return reply + "You must be a bot owner to promote users to admins.";
    } else if (await message.client.isAdmin(userId)) {
  ...
  _: async (content, reply, message, edited) => {
    ...
    '--.____  '--------.______       _.----.-----./      :/
      ...
      if (res.status == 200) {
        const repositories = (await res.json()).filter((repo) => !repo.private);
